...
if {[is_project_open]} {
	if {[string compare $quartus(project) "LogicalStep_Lab5"]} {
		puts "Project LogicalStep_Lab5 is not open"
		set make_assignments 0
	}
} else {
	# Only open if not already open
	if {[project_exists LogicalStep_Lab5]} {
		project_open -revision LogicalStep_Lab5_top LogicalStep_Lab5
	} else {
		project_new -revision LogicalStep_Lab5_top LogicalStep_Lab5
	}
...
#	set_global_assignment -name STRATIX_DEVICE_IO_STANDARD "2.5 V"
	...
	set_global_assignment -name SDC_FILE LogicalStep_Lab5_top.out.sdc

	set_global_assignment -name VHDL_FILE Lab5_Moore_SM.vhd
	set_global_assignment -name SOURCE_FILE LogicalStep_Lab5_top.vhd
	set_global_assignment -name SOURCE_FILE segment7_mux.vhd
	set_global_assignment -name VHDL_FILE cycle_generator.vhd
